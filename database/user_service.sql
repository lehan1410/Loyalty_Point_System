...
-- Trigger tự động sinh referral_code khi insert user mới
DELIMITER $$
...
-- Sinh referral_code cho user cũ chưa có
UPDATE Users
...
-- Đồng bộ created_at của user_profile theo Users
...
-- Update membertype nếu cần
UPDATE Customer SET membertype = 1 WHERE user_id IN (1, 2);
UPDATE Customer SET membertype = 2 WHERE user_id = 3;
